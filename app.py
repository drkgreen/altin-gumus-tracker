...
# Düzeltmeler: Versiyon bilgisi + LocalStorage fix
    ...
    <script>
        ...
        function savePortfolio() {
            try {
                ...
                console.log('Portföy kaydedildi:', { 
                    gold: goldAmount, 
                    silver: silverAmount,
                    timestamp: new Date().toLocaleTimeString()
                });
                
                // Test için localStorage'ı kontrol et
                console.log('Kaydedilen değerler kontrol:', {
                    goldStored: localStorage.getItem('portfolioGold'),
                    silverStored: localStorage.getItem('portfolioSilver')
                });
                
            } catch (e) {
                console.log('LocalStorage kaydetme hatası:', e);
                console.log('LocalStorage desteklenmiyor veya dolu olabilir');
            }
        }
        ...
        function loadPortfolio() {
            try {
                ...
                console.log('LocalStorage kontrol:', { savedGold, savedSilver });
                
                if (savedGold !== null && savedGold !== '' && savedGold !== 'null') {
                    document.getElementById('goldAmount').value = savedGold;
                    console.log('Altın miktarı yüklendi:', savedGold);
                }
                if (savedSilver !== null && savedSilver !== '' && savedSilver !== 'null') {
                    document.getElementById('silverAmount').value = savedSilver;
                    console.log('Gümüş miktarı yüklendi:', savedSilver);
                }
                
                // Yükleme sonrası hesapla
                setTimeout(() => {
                    calculatePortfolio();
                }, 100);
                
            } catch (e) {
                console.log('LocalStorage yükleme hatası:', e);
            }
        }
