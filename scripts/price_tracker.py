...
on:
  schedule:
    ...
    # Gece optimizasyon işi - her gece 02:50 Türkiye saati
    # 23:50 UTC = 02:50 Türkiye saati (UTC+3)
    - cron: '50 23 * * *'
  ...
  metal-tracker:
    ...
    - name: Determine operation based on time
      run: |
        ...
        # Gece optimizasyon kontrolü (02:50 Türkiye saati)
        if [ $TURKEY_HOUR -eq 2 ] && [ $TURKEY_MINUTE -eq 50 ]; then
    ...
    - name: Print operation summary
      run: |
        ...
        if [ $TURKEY_HOUR -eq 2 ] && [ $TURKEY_MINUTE -eq 50 ]; then
          ...
          echo "   🌙 02:50 TR (Gece optimizasyonu)"
        else
