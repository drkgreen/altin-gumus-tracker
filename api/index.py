    ...
    try:
        ...
        if today_records:
            ...
            for i, record in enumerate(sorted_records):
                ...
                hourly_data.append({"time": time_label, "gold_price": record["gold_price"], "silver_price": record["silver_price"], "change_percent": change_percent, "optimized": False, "is_peak": False})
        return hourly_data
    except:
        return []
...
def get_daily_optimized_data():
    try:
        ...
        for i, day_data in enumerate(daily_temp):
            ...
            daily_data.append({"time": f"{day_data['time']}", "gold_price": day_data["gold_price"], "silver_price": day_data["silver_price"], "change_percent": change_percent, "optimized": True, "peak_time": day_data["peak_time"], "portfolio_value": day_data["portfolio_value"], "is_peak": True})
        daily_data.reverse()
        return daily_data
    except:
        return []
...
def get_monthly_optimized_data():
    try:
        ...
        for i, month_data in enumerate(monthly_temp):
            ...
            monthly_data.append({"time": f"{month_data['time']}", "gold_price": month_data["gold_price"], "silver_price": month_data["silver_price"], "change_percent": change_percent, "optimized": True, "peak_time": month_data["peak_time"], "peak_date": month_data["peak_date"], "portfolio_value": month_data["portfolio_value"], "is_peak": True})
...
let goldChart=null,silverChart=null,portfolioChart=null;
...
async function fetchPrice(){const refreshBtn=document.getElementById('refreshBtn');try{refreshBtn.style.transform='rotate(360deg)';const[goldResponse,silverResponse,tableResponse]=await Promise.all([fetch('/api/gold-price'),fetch('/api/silver-price'),fetch('/api/table-data')]);const goldData=await goldResponse.json(),silverData=await silverResponse.json(),tableDataResult=await tableResponse.json();if(goldData.success){let cleaned=goldData.price.replace(/[^\d,]/g,'');currentGoldPrice=parseFloat(cleaned.replace(',','.'))}if(silverData.success){let cleaned=silverData.price.replace(/[^\d,]/g,'');currentSilverPrice=parseFloat(cleaned.replace(',','.'))}if(tableDataResult.success){tableData=tableDataResult.data;updateCharts()}document.getElementById('headerTime').textContent=new Date().toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});updatePortfolio()}catch(error){console.error('Fetch price error:',error)}finally{setTimeout(()=>refreshBtn.style.transform='rotate(0deg)',500)}}

function switchPeriod(period){currentPeriod=period;document.querySelectorAll('.period-tab').forEach(tab=>tab.classList.remove('active'));document.getElementById(period+'Tab').classList.add('active');updateCharts()}

function updateCharts(){if(!tableData||!tableData[currentPeriod])return;updateStatistics();const data=tableData[currentPeriod];if(data.length===0)return;const labels=data.map(item=>item.time);const goldPrices=data.map(item=>item.gold_price);const silverPrices=data.map(item=>item.silver_price);const portfolioValues=data.map(item=>(goldAmount*item.gold_price)+(silverAmount*item.silver_price));const peakIndices=data.map((item,idx)=>item.is_peak?idx:null).filter(idx=>idx!==null);createOrUpdateChart('goldChart','goldChart','Altın Fiyatı (₺)',labels,goldPrices,'#fbbf24','#f59e0b',peakIndices);createOrUpdateChart('silverChart','silverChart','Gümüş Fiyatı (₺)',labels,silverPrices,'#94a3b8','#64748b',peakIndices);createOrUpdateChart('portfolioChart','portfolioChart','Portföy Değeri (₺)',labels,portfolioValues,'#60a5fa','#3b82f6',peakIndices)}

function createOrUpdateChart(chartVar,canvasId,label,labels,data,borderColor,backgroundColor,peakIndices){const canvas=document.getElementById(canvasId);if(!canvas)return;if(window[chartVar]){window[chartVar].destroy()}const segmentColors=[];for(let i=1;i<data.length;i++){if(data[i]>data[i-1]){segmentColors.push('#22c55e')}else if(data[i]<data[i-1]){segmentColors.push('#ef4444')}else{segmentColors.push(borderColor)}}const pointStyles=labels.map((_,idx)=>peakIndices.includes(idx)?'star':'circle');const pointRadii=labels.map((_,idx)=>peakIndices.includes(idx)?8:3);const pointBorderWidths=labels.map((_,idx)=>peakIndices.includes(idx)?2:1);window[chartVar]=new Chart(canvas,{type:'line',data:{labels:labels,datasets:[{label:label,data:data,borderColor:borderColor,backgroundColor:backgroundColor+'40',borderWidth:2,fill:false,tension:0.3,pointBackgroundColor:borderColor,pointBorderColor:'#ffffff',pointBorderWidth:pointBorderWidths,pointRadius:pointRadii,pointStyle:pointStyles,pointHoverRadius:6,segment:{borderColor:ctx=>{const idx=ctx.p0DataIndex;return segmentColors[idx]||borderColor}}}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(15,23,42,0.95)',titleColor:'#60a5fa',bodyColor:'#e2e8f0',borderColor:'rgba(59,130,246,0.3)',borderWidth:1,padding:12,displayColors:false,callbacks:{title:ctx=>ctx[0].label,label:ctx=>{const value=ctx.parsed.y;const formatted=new Intl.NumberFormat('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}).format(value)+' ₺';const isPeak=peakIndices.includes(ctx.dataIndex);return isPeak?`${formatted} ⭐ PEAK`:formatted}}}},scales:{x:{grid:{color:'rgba(59,130,246,0.1)',drawBorder:false},ticks:{color:'rgba(226,232,240,0.7)',font:{size:10}}},y:{grid:{color:'rgba(59,130,246,0.1)',drawBorder:false},ticks:{color:'rgba(226,232,240,0.7)',font:{size:10},callback:value=>new Intl.NumberFormat('tr-TR',{minimumFractionDigits:0,maximumFractionDigits:0}).format(value)}}},interaction:{intersect:false,mode:'index'}}})}
