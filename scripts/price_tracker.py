    ...
    inputs:
      operation:
        ...
        options:
          ...
          - retroactive
      days:
        description: 'Geriye dönük kaç gün (retroactive için)'
        required: false
        default: '7'
        type: string
    ...
    - name: Determine operation based on time
      run: |
        ...
        if [ "${{ github.event_name }}" == "workflow_dispatch" ]; then
          echo "🎯 Manuel tetikleme algılandı"
          echo "📋 İşlem: ${{ github.event.inputs.operation }}"
          
          if [ "${{ github.event.inputs.operation }}" == "retroactive" ]; then
            echo "🔄 Geriye dönük optimizasyon başlatılıyor..."
            echo "📅 Son ${{ github.event.inputs.days }} gün optimize edilecek"
            python price_tracker.py --retroactive --days ${{ github.event.inputs.days }}
          elif [ "${{ github.event.inputs.operation }}" == "optimize" ]; then
            echo "🌙 Manuel optimizasyon başlatılıyor..."
            python price_tracker.py --optimize
          else
            echo "📊 Manuel veri toplama başlatılıyor..."
            python price_tracker.py --collect
          fi
          exit 0
        fi
